...
async def initialize_assistant(request: Request):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to initialize assistant"
            )
...
async def upload_database_content(
    ...
    data_type: str = Query(
        "all", description="Type of data to upload: publications, innovations, or all"
    ),
    limit: int = Query(
        100, ge=1, le=500, description="Maximum number of records to upload"
    ),
):
        ...
        if chat_request.innovation_id:
            ...
            if innovation_data:
                enhanced_message = await pinecone_assistant_service.enhance_message_with_innovation_context(
                    message=chat_request.message, innovation=innovation_data
                )

        response = await pinecone_assistant_service.chat(
            message=enhanced_message, conversation_id=chat_request.conversation_id
        )
...
async def get_domain_insights(
    request: Request, domain: str = "artificial intelligence"
):
...
async def get_assistant_status(request: Request):
    ...
    try:
        ...
        innovation_count = (
            innovations_response.count if innovations_response.count else 0
        )
...
async def get_innovation_context(innovation_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        publications_response = (
            supabase.table("innovation_publications")
        ...
        organizations_response = (
            supabase.table("innovation_organizations")
        ...
        logger.info(
            f"Retrieved context for innovation {innovation_id}: {len(related_publications)} publications, {len(related_organizations)} organizations, {len(related_fundings)} fundings"
        )
    ...
    try:
        ...
        if data_type in ["publications", "all"]:
            ...
            for pub in publications:
                try:
                    result = (
                        await pinecone_assistant_service.upload_publication_content(pub)
                    )
        ...
        if data_type in ["innovations", "all"]:
            ...
            for innovation in innovations:
                try:
                    result = await pinecone_assistant_service.upload_innovation_content(
                        innovation
                    )
                    if "error" not in result:
                        uploaded_count += 1
                    ...
                    logger.error(
                        f"Error uploading innovation {innovation.get('id')}: {e}"
                    )
                    continue
        ...
        logger.info(
            f"Assistant data upload job completed: {uploaded_count} documents uploaded"
        )
