...
async def get_intelligence_missing_data_map(request: Request):
    ...
    try:
        ...
        for table_name, schema in enrichment_schema.items():
            try:
                ...
                if table_name == "intelligence_reports":
                    ...
                    try:
                        response = (
                            supabase.table(table_name).select("*").limit(100).execute()
                        )
        ...
        return {
            ...
            "summary": {
                ...
                "total_records_analyzed": sum(
                    table.get("total_records", 0) for table in missing_data_map.values()
                ),
                "intelligence_table_exists": "intelligence_reports" in missing_data_map
                and missing_data_map["intelligence_reports"].get("total_records", 0)
                > 0,
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate missing data map: {str(e)}"
        )
...
async def get_enrichment_gaps_analysis(request: Request):
    ...
    try:
        ...
        try:
            reports_response = (
                supabase.table("intelligence_reports").select("*").execute()
            )
            intelligence_reports = (
                reports_response.data if reports_response.data else []
            )
        except Exception:
        ...
        gaps_analysis = {
            ...
            "critical_missing_data": identify_critical_missing_data(
                publications, innovations, intelligence_reports
            ),
            "enrichment_priority": calculate_enrichment_priority(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze enrichment gaps: {str(e)}"
        )
...
def analyze_table_completeness(records: List[Dict], schema: Dict) -> Dict[str, Any]:
    ...
    for field in all_fields:
        complete_count = sum(
            1 for record in completeness_matrix if record.get(field, False)
        )
    ...
    overall_completeness = (
        (total_complete_values / total_possible_values) * 100
    ...
    return {
        ...
        "core_fields_completeness": calculate_section_completeness(
            field_completeness, schema["core_fields"]
        ),
        "enrichment_fields_completeness": calculate_section_completeness(
            field_completeness, schema["enrichment_fields"]
        ),
    }


def calculate_section_completeness(
    field_completeness: Dict, fields: List[str]
) -> float:
...
def analyze_publication_enrichment_gaps(publications: List[Dict]) -> Dict[str, Any]:
    ...
    for pub in publications:
        ...
        if (
            not pub.get("extracted_technologies")
            or len(pub.get("extracted_technologies", [])) == 0
        ):
...
def analyze_innovation_enrichment_gaps(innovations: List[Dict]) -> Dict[str, Any]:
    ...
    for innovation in innovations:
        if (
            not innovation.get("ai_techniques_used")
            or len(innovation.get("ai_techniques_used", [])) == 0
        ):
            gaps["ai_techniques_missing"] += 1
        if not innovation.get("technical_approach"):
            gaps["technical_approach_missing"] += 1
        if (
            not innovation.get("performance_metrics")
            or len(innovation.get("performance_metrics", {})) == 0
        ):
            gaps["performance_metrics_missing"] += 1
        if (
            not innovation.get("impact_data")
            or len(innovation.get("impact_data", {})) == 0
        ):
    ...
    return {
        ...
        "recent_reports": len(
            [
                ...
                if r.get("generation_timestamp")
                and datetime.fromisoformat(r["generation_timestamp"])
                > datetime.now() - timedelta(days=7)
            ]
        )
        if intelligence_reports
        else 0,
        "intelligence_gap_severity": "critical"
        if len(intelligence_reports) == 0
        else "moderate",
    }


def identify_critical_missing_data(
    publications: List[Dict], innovations: List[Dict], reports: List[Dict]
) -> List[Dict[str, Any]]:
    ...
    if publications:
        enriched_publications = sum(
            1
            for p in publications
            if p.get("development_stage") or p.get("extracted_technologies")
        )
    ...
    if innovations:
        technical_innovations = sum(
            1
            for i in innovations
            if i.get("ai_techniques_used") or i.get("technical_approach")
        )
...
def calculate_enrichment_priority(
    publications: List[Dict], innovations: List[Dict]
) -> List[Dict[str, Any]]:
    ...
    if publications:
        recent_publications = [
            ...
            and datetime.strptime(p["publication_date"], "%Y-%m-%d")
...
def generate_actionable_insights(gaps_analysis: Dict) -> List[str]:
    ...
    for gap in critical_gaps:
        if gap.get("severity") == "critical":
            insights.append(
                f"Critical: {gap.get('description')} - {gap.get('recommended_action')}"
            )
    ...
    if enrichment_priorities:
        ...
        insights.append(
            f"Top Priority: {top_priority.get('task')} - {top_priority.get('justification')}"
        )
