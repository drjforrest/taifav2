...
async def trigger_citation_extraction(
    ...
    batch_size: int = Query(
        50, ge=10, le=200, description="Number of publications to process per batch"
    ),
):
    ...
    try:
        if not await citations_service.initialize():
            raise HTTPException(
                status_code=500, detail="Failed to initialize citations service"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to start citation extraction"
        )


@router.get("/citations/impact-scores")
@limiter.limit("10/minute")
async def calculate_impact_scores(request: Request, background_tasks: BackgroundTasks):
    """Calculate publication impact scores"""
    try:
        if not await citations_service.initialize():
            raise HTTPException(
                status_code=500, detail="Citations service not available"
            )
...
@limiter.limit("10/minute")
async def map_knowledge_flows(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to start knowledge flow mapping"
        )
...
async def trigger_metadata_enhancement(
    ...
    background_tasks: BackgroundTasks,
    batch_size: int = Query(
        100, ge=50, le=500, description="Number of publications to process per batch"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to start metadata enhancement"
        )
...
async def get_publication_intelligence_report(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get publication intelligence"
        )
...
async def get_first_movers(
    ...
    domain: Optional[str] = Query(
        None, description="Filter by domain (e.g., 'agriculture', 'healthcare')"
    ),
...
async def get_collaboration_opportunities(
    ...
    country: Optional[str] = Query(
        None, description="Focus country for collaboration opportunities"
    ),
    ...
    try:
        ...
        connections_response = (
            supabase.table("institutional_connections").select("*").execute()
        )
        affiliations_response = (
            supabase.table("author_affiliations").select("*").execute()
        )
        ...
        return {
            ...
            "analysis_scope": {
                ...
                "institutions_analyzed": sum(
                    len(insts) for insts in country_institutions.values()
                ),
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get collaboration opportunities"
        )
    ...
    try:
        ...
        for pub in publications:
            ...
            commercial_score = sum(
                1 for indicator in commercial_indicators if indicator in abstract
            )
                ...
                commercialization_candidates.append(
                    {
                        ...
                        "abstract_preview": pub.get("abstract", "")[:200] + "..."
                        if pub.get("abstract")
                        else "",
                    }
                )
        ...
        commercialization_candidates.sort(
            key=lambda x: x["readiness_score"], reverse=True
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get commercialization-ready research"
        )
...
async def get_research_leaders(
    ...
    metric: str = Query(
        "influence_score",
        description="Ranking metric: influence_score, citation_count, h_index_contribution",
    ),
    ...
    try:
        ...
        response = (
            supabase.table("publications")
            .select("id, title, authors, impact_metrics, publication_date")
            .execute()
        )
                ...
                if isinstance(author, str):
                    if author not in author_metrics:
                        author_metrics[author] = {
                            ...
                            "institution": author_institutions.get(author, {}).get(
                                "institution", "Unknown"
                            ),
                            "country": author_institutions.get(author, {}).get(
                                "country", "Unknown"
                            ),
                        }

                    author_metrics[author]["publication_count"] += 1
                    author_metrics[author]["total_influence"] += impact_metrics.get(
                        "influence_score", 0
                    )
                    author_metrics[author]["total_citations"] += impact_metrics.get(
                        "citation_count", 0
                    )
                    author_metrics[author]["h_index_contribution"] += (
                        impact_metrics.get("h_index_contribution", 0)
                    )
        ...
        for author_data in author_metrics.values():
            ...
            if institution != "Unknown":
                ...
                institution_metrics[institution]["publication_count"] += author_data[
                    "publication_count"
                ]
                institution_metrics[institution]["total_influence"] += author_data[
                    "total_influence"
                ]
                institution_metrics[institution]["total_citations"] += author_data[
                    "total_citations"
                ]
        ...
        top_authors = sorted(
            author_metrics.values(), key=lambda x: x.get(sort_key, 0), reverse=True
        )[:limit]

        top_institutions = sorted(
            institution_metrics.values(), key=lambda x: x.get(sort_key, 0), reverse=True
        )[:limit]
...
async def run_citation_extraction_job(batch_size: int):
    ...
    try:
        ...
        citations = await citations_service.extract_citations_from_publications(
            batch_size
        )

        logger.info(
            f"Citation extraction job completed: {len(citations)} citations extracted"
        )
...
async def run_impact_scoring_job():
    ...
    try:
        ...
        logger.info(
            f"Impact scoring job completed: {len(impact_metrics)} publications scored"
        )
...
async def run_knowledge_flow_mapping():
    ...
    try:
        ...
        if not await citations_service.initialize():
            logger.error(
                "Failed to initialize citations service for knowledge flow mapping"
            )
            return
...
async def run_metadata_enhancement_job(batch_size: int):
    ...
    try:
        ...
        results = await enhanced_publication_service.enhance_publication_metadata(
            batch_size
        )

        total_processed = results.get("total_processed", 0)
        logger.info(
            f"Metadata enhancement job completed: {total_processed} publications processed"
        )
