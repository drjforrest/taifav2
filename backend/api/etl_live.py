        ...
        return {
            ...
            "data": {
                ...
                "pipelines": {
                    "academic_pipeline": {
                        "status": "running"
                        if unified_status.academic_pipeline_active
                        else "idle",
                        "last_run": unified_status.last_academic_run,
                        "items_processed": next(
                            (
                                job["items_processed"]
                                for job in dashboard_data["job_statuses"]
                                if job["name"] == "academic_pipeline"
                            ),
                            0,
                        ),
                        "errors": next(
                    ...
                    "news_pipeline": {
                        "status": "running"
                        if unified_status.news_pipeline_active
                        else "idle",
                    ...
                    "discovery_pipeline": {
                        "status": "running"
                        if unified_status.serper_pipeline_active
                        else "idle",
                    ...
                    "enrichment_pipeline": {
                        "status": "running"
                        if unified_status.enrichment_pipeline_active
                        else "idle",
                        ...
                        "items_processed": next(
                            (
                                job["items_processed"]
                                for job in dashboard_data["job_statuses"]
                                if job["name"] == "enrichment_pipeline"
                            ),
                            0,
                        ),
                        "errors": next(
...
async def get_development_configuration():
    ...
    try:
        return {
            "success": True,
            "skip_expensive_ops": (
                settings.DISABLE_AI_ENRICHMENT
                or settings.DISABLE_EXTERNAL_SEARCH
                or settings.DISABLE_RSS_MONITORING
                or settings.DISABLE_ACADEMIC_SCRAPING
            ),
...
async def trigger_news_pipeline(background_tasks: BackgroundTasks):
    ...
    if not etl_monitor.start_job("news_pipeline"):
        return ETLResponse(success=False, message="News pipeline is already running")
...
async def trigger_academic_pipeline(
    background_tasks: BackgroundTasks, days_back: int = 3, max_results: int = 10
):
    ...
    if not etl_monitor.start_job("academic_pipeline"):
        return ETLResponse(
            success=False, message="Academic pipeline is already running"
        )
    ...
    return ETLResponse(
        ...
        data={
            "pipeline": "academic",
            "status": "started",
            "params": {"days_back": days_back, "max_results": max_results},
        },
    )
...
async def trigger_discovery_pipeline(
    background_tasks: BackgroundTasks, query: str = "African AI innovation"
):
    ...
    if not etl_monitor.start_job("serper_pipeline"):
        return ETLResponse(
            success=False, message="Discovery pipeline is already running"
        )
...
async def trigger_enrichment_pipeline(
    ...
    if not etl_monitor.start_job("enrichment_pipeline"):
        return ETLResponse(
            success=False, message="AI enrichment pipeline is already running"
        )
    ...
    return ETLResponse(
        ...
        data={
            ...
            or ["Nigeria", "Kenya", "South Africa", "Ghana", "Egypt"],
...
async def get_recent_news_results(limit: int = 10):
    ...
    try:
        ...
        results = await vector_service.search_similar(
            "African technology news", top_k=limit
        )
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving news results: {str(e)}"
        )
...
async def get_recent_academic_results(limit: int = 10):
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving academic results: {str(e)}"
        )
...
async def get_recent_innovations(limit: int = 10):
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving innovations: {str(e)}"
        )
...
async def get_recent_enrichment_results(limit: int = 10):
    ...
    try:
        ...
        vector_service = await get_vector_service()
        results = await vector_service.search_similar(
            "AI intelligence report", top_k=limit
        )
                ...
                reports.append(
                    {
                        ...
                        "generation_timestamp": result.metadata.get(
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving AI enrichment results: {str(e)}"
        )
...
async def run_news_pipeline():
    ...
    try:
        ...
        etl_monitor.complete_job(
            "news_pipeline", True, runtime, items_processed, metrics=metrics
        )
...
async def run_academic_pipeline(days_back: int = 3, max_results: int = 10):
    ...
    try:
        logger.info(
            f"Starting academic pipeline (days_back={days_back}, max_results={max_results})"
        )
        ...
        if papers:
            ...
            for paper in papers:
                try:
                    ...
                    logger.warning(
                        f"Error inserting paper {paper.title}: {insert_error}"
                    )
                    items_failed += 1
        ...
        # Complete job with metrics
        etl_monitor.complete_job(
            "academic_pipeline", True, runtime, items_processed, metrics=metrics
        )
...
async def run_enrichment_pipeline(
    ...
    try:
        logger.info(
            f"Starting AI enrichment pipeline with {provider} using {len(intelligence_types)} intelligence types"
        )

        # Route to appropriate provider
        if provider == "perplexity":
            reports_count = await run_perplexity_enrichment(
                intelligence_types,
                time_period,
                geographic_focus,
                enable_snowball_sampling,
            ...
            # Complete job with metrics
            etl_monitor.complete_job(
                "enrichment_pipeline", True, runtime, reports_count, metrics=metrics
            )
            logger.info("AI enrichment pipeline completed successfully")
        else:
            logger.error(f"Unsupported enrichment provider: {provider}")
            etl_monitor.complete_job(
                "enrichment_pipeline", False, 0, 0, f"Unsupported provider: {provider}"
            )
...
async def run_perplexity_enrichment(
    ...
    if reports:
        ...
        for report in reports:
            try:
                ...
                logger.warning(
                    f"Could not store report {report.report_id} in database: {db_error}"
                )
    ...
    if enable_snowball_sampling and reports:
        try:
            ...
            total_citations = sum(
                len(report.extracted_citations or []) for report in reports
            )

            if total_citations > 0:
                logger.info(
                    f"Starting snowball sampling with {total_citations} extracted citations"
                )

                # Run snowball sampling with conservative parameters
                sampling_results = await run_snowball_sampling(
                    max_depth=2, max_citations=15
                )

                logger.info(
                    f"Snowball sampling completed: {sampling_results.get('new_discoveries', 0)} new discoveries"
                )
                logger.info(
                    f"Processed {sampling_results.get('citations_processed', 0)} citations in {sampling_results.get('duration', 0):.1f} seconds"
                )
            else:
            ...
            logger.warning(
                f"Snowball sampling failed (continuing with main pipeline): {e}"
            )
...
async def get_scheduler_status():
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving scheduler status: {str(e)}"
        )
...
async def configure_scheduler(
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error configuring scheduler: {str(e)}"
        )
...
async def start_scheduler(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return ETLResponse(success=False, message=f"Error starting scheduler: {str(e)}")
...
async def stop_scheduler(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return ETLResponse(success=False, message=f"Error stopping scheduler: {str(e)}")
...
async def get_null_cache_stats():
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving null cache stats: {str(e)}"
        )
...
async def clear_null_cache(data_source: Optional[str] = None):
    ...
    try:
        from services.null_result_cache import DataSource, null_result_cache
        ...
        return ETLResponse(
            success=False, message=f"Error clearing null cache: {str(e)}"
        )
...
async def cleanup_expired_null_cache():
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error cleaning up null cache: {str(e)}"
        )
...
async def get_unified_cache_stats():
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving unified cache stats: {str(e)}"
        )
...
async def invalidate_cache_pattern(pattern: str):
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error invalidating cache pattern: {str(e)}"
        )
...
async def cleanup_unified_cache():
    ...
    try:
        ...
        return ETLResponse(
            success=False, message=f"Error cleaning up unified cache: {str(e)}"
        )


@router.post("/cache/unified/warm")
async def warm_unified_cache(
    background_tasks: BackgroundTasks, warming_tasks: List[Dict[str, Any]] = None
):
    ...
    try:
        ...
        if not warming_tasks:
            warming_tasks = [
                ...
                },
                {
                    "data_source": "serper",
                    "query_params": {
                        "query": "African fintech innovation",
                        "num_results": 20,
                    },
                    "cache_type": "positive",
                },
                {
                    ...
                    "query_params": {
                        "prompt": "Latest AI developments in Nigeria",
        ...
        return ETLResponse(
            success=False, message=f"Error starting cache warming: {str(e)}"
        )
    ...
    try:
        ...
        performance_data = {
            "unified_cache": {
                ...
                "memory_cache_hits": unified_stats.get("performance", {}).get(
                    "memory_hits", 0
                ),
                "redis_cache_hits": unified_stats.get("performance", {}).get(
                    "redis_hits", 0
                ),
                "cache_sets": unified_stats.get("performance", {}).get("sets", 0),
                "compressions": unified_stats.get("compression_stats", {}).get(
                    "compressions", 0
                ),
            },
            "null_cache": {
            ...
            "efficiency_metrics": {
                "api_calls_saved": unified_stats.get("performance", {}).get("hits", 0)
                + null_stats.get("total_cached_items", 0),
                "cost_savings_estimate": (
                    unified_stats.get("performance", {}).get("hits", 0) * 0.02
                )
                + (null_stats.get("total_cached_items", 0) * 0.01),  # Rough estimate
            },
        }
        ...
        return ETLResponse(
            success=False, message=f"Error retrieving cache performance: {str(e)}"
        )
