...
class EnrichmentScheduler:
    ...
    def update_schedule(
        ...
        if interval_hours is not None:
            ...
            if self.schedule.last_run:
                ...
                self.schedule.next_run = datetime.now() + timedelta(
                    hours=interval_hours
                )
        ...
        logger.info(
            f"Updated enrichment schedule: interval={self.schedule.interval_hours}h, "
    ...
    def get_schedule_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_run": self.schedule.last_run.isoformat()
            if self.schedule.last_run
            else None,
            "next_run": self.schedule.next_run.isoformat()
            if self.schedule.next_run
            else None,
        ...
        try:
            while self.running:
                ...
                if (
                    ...
                    and self.schedule.next_run
                    and datetime.now() >= self.schedule.next_run
                ):
    ...
    async def _run_scheduled_enrichment(self):
        ...
        try:
            ...
            if self.schedule.provider == "perplexity":
                ...
                logger.error(
                    f"Unsupported enrichment provider: {self.schedule.provider}"
                )
    ...
    async def _run_perplexity_enrichment(self):
        ...
        logger.info(
            f"Running scheduled Perplexity enrichment with {len(intel_types)} intelligence types"
        )
        ...
        async with PerplexityAfricanAIModule(api_key) as perplexity_module:
            ...
            logger.info(
                f"Scheduled enrichment generated {len(reports)} intelligence reports"
            )
    ...
    async def _store_reports(self, reports):
        ...
        try:
            ...
            from services.vector_service import VectorDocument, get_vector_service
            ...
            for report in reports:
                ...
                doc = VectorDocument(
                    ...
                    metadata={
                        ...
                        "innovations_mentioned_count": len(
                            report.innovations_mentioned
                        ),
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not store scheduled report {report.report_id} in database: {db_error}"
                    )
