...
class HistoricalTrendService:
    ...
    async def analyze_innovation_lifecycle(
        self, innovation_ids: Optional[List[str]] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            lifecycle_analysis = {
                ...
                "progression_patterns": self._identify_progression_patterns(
                    innovations
                ),
        ...
        for innovation in innovations:
            ...
            if innovation.get("fundings"):
                ...
                if isinstance(funding_data, list) and funding_data:
                    total_funding = sum(
                        f.get("amount", 0) for f in funding_data if f.get("amount")
                    )
    ...
    def _analyze_success_trajectories(self, innovations: List[Dict]) -> Dict:
        ...
        trajectories = {
            ...
            "geographic_distribution": self._analyze_geographic_success(
                successful_innovations
            ),
    ...
    def _analyze_development_timelines(self, innovations: List[Dict]) -> Dict:
        ...
        timeline_analysis = {
            "yearly_distribution": {
                str(year): len(innovations) for year, innovations in yearly_data.items()
            },
        ...
        for domain, dates in domain_timeline.items():
            if dates:
                ...
                emergence_data[domain] = {
                    ...
                    "recent_activity": len(
                        [d for d in dates if d > datetime.now() - timedelta(days=365)]
                    ),
                }
    ...
    def _analyze_domain_convergence(self, innovations: List[Dict]) -> Dict:
        ...
        top_convergences = sorted(
            convergence_patterns.items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def _analyze_failure_patterns(self, innovations: List[Dict]) -> Dict:
        ...
        for innovation in innovations:
            if (
                innovation.get("verification_status") == "pending"
                and not innovation.get("fundings")
                and innovation.get("creation_date")
            ):
                try:
                    creation_date = datetime.fromisoformat(
                        innovation["creation_date"].replace("Z", "+00:00")
                    )
                    if creation_date < datetime.now() - timedelta(
                        days=730
                    ):  # 2+ years old
                        stalled_count += 1
                except (ValueError, TypeError):
        ...
        return {
            ...
            "stall_rate": round(stalled_count / total_innovations, 3)
        ...
        if funding_data:
            return {
                ...
                "funding_ranges": {
                    ...
                    "100k_to_1m": len(
                        [f for f in funding_data if 100000 <= f < 1000000]
                    ),
    ...
    def _calculate_growth_trends(self, yearly_data: Dict) -> Dict:
        ...
        recent_growth = (
            counts[-1] / counts[-2] - 1 if len(counts) > 1 and counts[-2] > 0 else 0
        )
    ...
    def _identify_peak_years(self, yearly_data: Dict) -> List[int]:
        ...
        counts_by_year = {
            year: len(innovations) for year, innovations in yearly_data.items()
        }
        max_count = max(counts_by_year.values())
        peak_years = [
            year for year, count in counts_by_year.items() if count == max_count
        ]
                ...
                try:
                    ...
                    if date > recent_cutoff:
                        recent_domains.append(
                            innovation.get("innovation_type", "Other")
                        )
                except (ValueError, TypeError):
                    continue
    ...
    def _identify_mature_domains(self, innovations: List[Dict]) -> List[str]:
        ...
        mature_domains = [
            domain
            for domain, data in domain_maturity.items()
        ...
        return {
            "stages_distribution": {
                ...
                "progression_patterns": {
                    "typical_timeline": "Research to market: 2-5 years"
                },
                "success_trajectories": {
                    "success_rate": "~10% reach commercial success"
                },
    ...
    def _get_fallback_success_data(self) -> Dict:
        ...
        return {
            ...
            "failure_patterns": {
                "stall_rate": 0.3,
                "common_reasons": ["Funding gaps", "Market fit"],
            },
            "predictive_features": [
                "Team experience",
                "Market size",
