...
class InnovationLifecycleTracker:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Innovation Lifecycle Tracker service: {e}"
            )
            return False
    ...
    async def create_lifecycle_record(
        ...
        try:
            ...
            if response.data:
                logger.info(
                    f"Created lifecycle record for innovation {innovation_id}, stage {stage.value}"
                )
                return True
            else:
                logger.error(
                    f"Failed to create lifecycle record for innovation {innovation_id}"
                )
                return False

        except Exception as e:
            logger.error(
                f"Error creating lifecycle record for innovation {innovation_id}: {e}"
            )
            return False
    ...
    async def get_innovation_lifecycle(
        self, innovation_id: UUID
    ) -> List[LifecycleStageData]:
        ...
        try:
            ...
            for record in response.data:
                stage_data = LifecycleStageData(
                    ...
                    start_date=datetime.fromisoformat(record["stage_start_date"]).date()
                    if record.get("stage_start_date")
                    else None,
                    end_date=datetime.fromisoformat(record["stage_end_date"]).date()
                    if record.get("stage_end_date")
                    else None,
    ...
    async def get_lifecycle_metrics(self, innovation_id: UUID) -> LifecycleMetrics:
        ...
        try:
            ...
            creation_date = None
            if innovation_response.data and innovation_response.data[0].get(
                "creation_date"
            ):
                creation_date = datetime.fromisoformat(
                    innovation_response.data[0]["creation_date"]
                ).date()
            ...
            for stage_data in lifecycle_stages:
                ...
                if (
                    stage_data.stage == LifecycleStage.COMMERCIAL
                    and stage_data.end_date
                ):
            ...
            logger.error(
                f"Error calculating lifecycle metrics for innovation {innovation_id}: {e}"
            )
            return LifecycleMetrics()

    # INTEGRATION METHODS
    async def link_publication_to_innovation_lifecycle(
        self, publication_id: UUID, innovation_id: UUID
    ) -> bool:
        ...
        try:
            ...
            if lifecycle_response.data:
                ...
                update_data = {"key_milestones": ["Research paper published"]}
                if pub_date:
                ...
                response = (
                    self.supabase.table("innovation_lifecycles")
                    .update(update_data)
                    .eq("id", lifecycle_response.data[0]["id"])
                    .execute()
                )
            else:
            ...
            if response.data:
                logger.info(
                    f"Linked publication {publication_id} to innovation {innovation_id} lifecycle"
                )
                return True
            else:
                logger.error(
                    f"Failed to link publication {publication_id} to innovation {innovation_id} lifecycle"
                )
                return False
    ...
    async def get_lifecycle_analytics(
        ...
        try:
            ...
            completed_cycles = 0
            total_innovations = len(
                set(record["innovation_id"] for record in lifecycle_response.data)
            )

            # Get all innovations for completion rate calculation
            innovation_ids = [
                record["innovation_id"] for record in lifecycle_response.data
            ]
            if innovation_ids:
                innovation_response = (
                    self.supabase.table("innovations")
                    .select("id")
                    .in_("id", innovation_ids)
                    .eq("visibility", "public")
                    .execute()
                )
                total_innovations = (
                    len(innovation_response.data) if innovation_response.data else 0
                )
            ...
            for stage_name, durations in stage_durations.items():
                if durations:
                    avg_duration_by_stage[stage_name] = round(
                        sum(durations) / len(durations), 1
                    )

            # Calculate completion rates
            completion_rate = (
                (completed_cycles / max(1, total_innovations)) * 100
    ...
    async def get_time_to_market_analysis(
        self, country: Optional[str] = None, innovation_type: Optional[str] = None
    ) -> Dict[str, Any]:
                ...
                for lifecycle in lifecycles:
                    if lifecycle.get("stage") == "commercial":
                        if lifecycle.get("stage_end_date"):
                            last_stage_end = datetime.fromisoformat(
                                lifecycle["stage_end_date"]
                            ).date()
                        elif lifecycle.get("stage_start_date"):
                            last_stage_start = datetime.fromisoformat(
                                lifecycle["stage_start_date"]
                            ).date()
