...
from services.serper_service import SearchResult, SerperService


class SearchCondition(Enum):
    """Conditions that trigger targeted searches"""

    MISSING_FUNDING_WITH_MENTIONS = (
        "missing_funding_with_mentions"  # Has funding mentions but no amount
    )
    VERIFIED_INNOVATION_MISSING_WEBSITE = (
        "verified_innovation_missing_website"  # Verified but no website
    )
    HIGH_IMPACT_MISSING_METRICS = (
        "high_impact_missing_metrics"  # High user count mentions but no metrics
    )
    RECENT_NEWS_MISSING_DETAILS = (
        "recent_news_missing_details"  # Recent mentions but missing key details
    )
...
class TargetedSearchService:
    ...
    def should_trigger_search(
        self, innovation: Dict[str, Any], field_name: str
    ) -> SearchTriggering:
    ...
    def _check_funding_search_conditions(
        self, innovation: Dict[str, Any], text: str
    ) -> SearchTriggering:
        ...
        has_recorded_funding = bool(
            innovation.get("fundings") and len(innovation.get("fundings", [])) > 0
        )
        ...
        return SearchTriggering(
            False, "No funding search conditions met", 0.0, 0.0, 0.0
        )

    def _check_website_search_conditions(
        self, innovation: Dict[str, Any], text: str
    ) -> SearchTriggering:
        ...
        has_company_mentions = any(
            term in text for term in ["company", "startup", "platform", "app"]
        )
        ...
        return SearchTriggering(
            False, "No website search conditions met", 0.0, 0.0, 0.0
        )

    def _check_investment_search_conditions(
        self, innovation: Dict[str, Any], text: str
    ) -> SearchTriggering:
        ...
        has_investment_details = any(
            indicator in text for indicator in investment_indicators
        )
        ...
        return SearchTriggering(
            False, "No investment search conditions met", 0.0, 0.0, 0.0
        )

    def _check_metrics_search_conditions(
        self, innovation: Dict[str, Any], text: str
    ) -> SearchTriggering:
        ...
        has_metrics_mentions = any(
            indicator in text for indicator in metrics_indicators
        )
        has_recorded_metrics = bool(
            innovation.get("impact_metrics", {}).get("users_reached")
        )
        ...
        return SearchTriggering(
            False, "No metrics search conditions met", 0.0, 0.0, 0.0
        )

    def _check_contact_search_conditions(
        self, innovation: Dict[str, Any], text: str
    ) -> SearchTriggering:
        """Check conditions for contact information searches"""

        has_organizations = bool(
            innovation.get("organizations")
            and len(innovation.get("organizations", [])) > 0
        )
        has_individuals = bool(
            innovation.get("individuals") and len(innovation.get("individuals", [])) > 0
        )
        is_verified = innovation.get("verification_status") == "verified"
        ...
        return SearchTriggering(
            False, "No contact search conditions met", 0.0, 0.0, 0.0
        )

    async def perform_targeted_search(
        self, innovation: Dict[str, Any], field_name: str
    ) -> Optional[TargetedSearchResult]:
        ...
        if not trigger_check.should_trigger:
            logger.info(
                f"Search not triggered for {field_name}: {trigger_check.reason}"
            )
            return None
        ...
        logger.info(
            f"Performing targeted search for {field_name} (confidence: {trigger_check.confidence:.2f})"
        )
        ...
        try:
            ...
            # Perform search
            search_results = await self.serper_service.search_web(
                search_query, num_results=15
            )
    ...
    async def _extract_targeted_value(
        self, results: List[SearchResult], field_name: str, innovation: Dict[str, Any]
        ...
        if field_name == "funding_amount":
            ...
            return self._extract_website_url(
                combined_text, results, innovation.get("title", "")
            )
    ...
    def _extract_funding_amount(
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                confidence = self._calculate_funding_confidence(
                    match.group(), text, results
                )
    ...
    def _extract_website_url(
        self, text: str, results: List[SearchResult], innovation_name: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _extract_investment_details(
        self, text: str, results: List[SearchResult]
    ) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_funding_confidence(
        self, match_text: str, full_text: str, results: List[SearchResult]
    ) -> float:
        ...
        if any(
            word in match_text.lower() for word in ["raised", "secured", "completed"]
        ):
            base_confidence += 0.1
    ...
    def _calculate_url_relevance(self, domain: str, innovation_name: str) -> float:
        ...
        # Direct name match
        if name_lower in domain_lower or any(
            word in domain_lower for word in name_lower.split()
        ):
            return 0.9
    ...
    async def batch_targeted_search(
        self, innovations: List[Dict[str, Any]], max_searches: int = 20
    ) -> List[TargetedSearchResult]:
        """Perform batch targeted searches with budget constraints"""

        logger.info(
            f"Starting batch targeted search for {len(innovations)} innovations"
        )
            ...
            for field_name in field_priority:
                ...
                if self._is_field_missing(innovation, field_name):
                    ...
                    if result:
                        ...
                        logger.info(
                            f"Found {field_name} for {innovation.get('title')}: ${result.search_cost:.3f}"
                        )
        ...
        logger.info(
            f"Completed {searches_performed} targeted searches, found {len(results)} results"
        )
        return results
    ...
    def _is_field_missing(self, innovation: Dict[str, Any], field_name: str) -> bool:
        ...
        if field_name == "funding_amount":
            ...
            return not (
                innovation.get("fundings")
                and any(
                    funding.get("funder_name")
                    for funding in innovation.get("fundings", [])
                )
            )
            ...
            return not (
                innovation.get("organizations") or innovation.get("individuals")
            )
...
async def run_targeted_searches_for_innovations(
    innovations: List[Dict[str, Any]], max_searches: int = 20
) -> List[TargetedSearchResult]:
...
if __name__ == "__main__":
    ...
    async def test_targeted_search():
        ...
        async with TargetedSearchService() as search_service:
            ...
            funding_trigger = search_service.should_trigger_search(
                test_innovation, "funding_amount"
            )
            print(
                f"Funding search trigger: {funding_trigger.should_trigger} - {funding_trigger.reason}"
            )

            # Check website search conditions
            website_trigger = search_service.should_trigger_search(
                test_innovation, "website_url"
            )
            print(
                f"Website search trigger: {website_trigger.should_trigger} - {website_trigger.reason}"
            )
