...
class SerperService:
    ...
    async def search_web(
        ...
        try:
                ...
                if response.status == 200:
                    ...
                    if search_response.results:
                        ...
                        await cache_api_response(
                            DataSource.SERPER, cache_params, response_dict, 12.0
                        )
                        logger.info(
                            f"Cached Serper web search with {len(search_response.results)} results"
                        )
                    else:
                        # Cache null result for 6 hours if no results
                        await cache_null_response(
                            DataSource.SERPER, cache_params, "no_results", 6.0
                        )
            ...
            await cache_null_response(
                DataSource.SERPER, cache_params, "network_error", 0.5
            )  # 30 minutes
    ...
    async def search_news(
        self, query: str, num_results: int = 20, days_back: int = 30
    ) -> SerperSearchResponse:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/news", json=payload
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def search_scholar(
        self, query: str, num_results: int = 20, year_from: Optional[int] = None
    ) -> SerperSearchResponse:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/scholar", json=payload
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    def parse_web_results(
        ...
        return SerperSearchResponse(
            ...
            search_time=data.get("searchInformation", {}).get(
                "formattedSearchTime", 0.0
            ),
            timestamp=datetime.now(),
            search_type="web",
        )

    def parse_news_results(
        self, query: str, data: Dict[str, Any]
    ) -> SerperSearchResponse:
    ...
    async def search_african_ai_innovations(
        self,
        ...
        logger.info(
            f"Searching for African AI innovations: type={innovation_type}, country={country}"
        )
        ...
        for query in base_queries:
            try:
                ...
                news_results = await self.search_news(
                    query, results_per_query // 2, days_back=180
                )
        ...
        sorted_results = sorted(
            filtered_results, key=self.calculate_relevance_score, reverse=True
        )

        return sorted_results[:num_results]

    async def search_innovation_funding(
        self, days_back: int = 30, min_amount: Optional[float] = None
    ) -> List[SearchResult]:
    ...
    async def search_research_papers(
        self, keywords: List[str], year_from: int = 2020
    ) -> List[SearchResult]:
    ...
    def filter_african_innovation_results(
...
async def search_african_innovations(
    innovation_type: Optional[str] = None,
    country: Optional[str] = None,
    num_results: int = 50,
) -> List[SearchResult]:
    ...
    async with SerperService() as serper:
        return await serper.search_african_ai_innovations(
...
async def search_academic_papers(
    keywords: List[str], year_from: int = 2020
) -> List[SearchResult]:
...
if __name__ == "__main__":
    ...
    async def test_serper():
        ...
        innovations = await search_african_innovations(
            innovation_type="FinTech", country="Nigeria", num_results=10
        )
