...
from services.null_result_cache import DataSource
...
class UnifiedCacheService:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        self.ttl_settings = {
            ...
            (
                DataSource.ARXIV_API,
                CacheType.POSITIVE,
            ): 48,  # Research papers change rarely
    ...
    def _compress_content(self, content: Any) -> Tuple[bytes, CompressionLevel]:
        ...
        if compression_level != CompressionLevel.NONE:
            ...
            logger.debug(
                f"Compressed {content_size} bytes to {len(compressed)} bytes ({compression_level.value})"
            )

        return compressed, compression_level

    def _decompress_content(
        self, compressed_data: bytes, compression_level: CompressionLevel
    ) -> Any:
        """Decompress content"""
    ...
    async def get(
        ...
        data_source: DataSource,
        query_params: Dict[str, Any],
        cache_type: CacheType = CacheType.POSITIVE,
    ) -> Optional[Any]:
        """Get cached result"""
    ...
    async def set(
        ...
        try:
            ...
            await self.redis.setex(
                cache_key, ttl_seconds, pickle.dumps(entry.to_dict())
            )
            ...
            self.stats["sets"] += 1
            logger.debug(
                f"Cached {data_source.value} {cache_type.value} result for {ttl_hours}h"
            )
            return True
    ...
    async def warm_cache(self, warming_tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
                ...
                if data_source == DataSource.SERPER:
                    ...
                    async with SerperService() as serper:
                        ...
                        if result and result.results:
                            await self.set(
                                data_source, query_params, result.results, cache_type
                            )
        ...
        logger.info(
            f"Cache warming completed: {results['successful']} successful, {results['failed']} failed"
        )
        return results
...
async def cache_api_response(
    ...
    async with unified_cache as cache:
        return await cache.set(
            data_source, query_params, response_data, CacheType.POSITIVE, ttl_hours
        )
...
async def cache_null_response(
    ...
    async with unified_cache as cache:
        ...
        return await cache.set(
            data_source, query_params, null_data, CacheType.NEGATIVE, ttl_hours
        )
...
if __name__ == "__main__":
    ...
    async def test_unified_cache():
        ...
        async with UnifiedCacheService() as cache:
            ...
            await cache.set(
                DataSource.SERPER, test_params, test_data, CacheType.POSITIVE
            )

            # Test getting cached result
            cached_result = await cache.get(
                DataSource.SERPER, test_params, CacheType.POSITIVE
            )
