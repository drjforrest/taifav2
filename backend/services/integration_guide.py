...
class BackfillIntegrationService:
    ...
    async def get_innovations_needing_backfill(
        self, limit: int = 50
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.data:
                logger.info(
                    f"Found {len(response.data)} innovations for potential backfilling"
                )
    ...
    async def run_scheduled_backfill(self) -> Dict[str, Any]:
        ...
        try:
            ...
            processed_jobs = await ai_backfill_service.run_scheduled_backfill(
                max_jobs=min(10, len(high_priority_jobs))
            )
            ...
            targeted_results = await run_targeted_searches_for_innovations(
                innovations, max_searches=15
            )

            # Phase 3: Apply results to database
            logger.info("Phase 3: Applying backfill results to database")
            update_results = await self.apply_backfill_results(
                processed_jobs, targeted_results
            )
            ...
            # Calculate costs
            total_cost = sum(job.total_cost for job in processed_jobs) + sum(
                result.search_cost for result in targeted_results
            )
            ...
            summary = {
                ...
                "cost_savings_estimate": self.estimate_manual_cost_savings(
                    processed_jobs, targeted_results
                ),
            }
    ...
    async def apply_backfill_results(
        self, backfill_jobs: List, targeted_results: List
    ) -> Dict[str, Any]:
        ...
        for job in backfill_jobs:
            if job.status.value == "completed" and job.results:
                try:
                    ...
                    logger.error(
                        f"Error applying backfill for {job.innovation_id}: {e}"
                    )
                    failed_updates += 1
        ...
        for result in targeted_results:
            try:
                update_data = await self.create_database_update_from_targeted_result(
                    result
                )
                if update_data:
                    await self.apply_innovation_updates(
                        result.innovation_id, update_data
                    )
                    successful_updates += 1
                    logger.info(
                        f"Applied targeted search result for {result.innovation_id}: {result.field_name}"
                    )

            except Exception as e:
                logger.error(
                    f"Error applying targeted result for {result.innovation_id}: {e}"
                )
                failed_updates += 1
    ...
    async def create_database_update_from_targeted_result(
        self, result
        ...
        if result.field_name == "funding_amount" and isinstance(
            result.extracted_value, dict
        ):
            funding_data = {
        ...
        elif result.field_name == "user_metrics" and isinstance(
            result.extracted_value, dict
        ):
    ...
    async def apply_innovation_updates(
        self, innovation_id: str, updates: Dict[str, Any]
    ):
        ...
        try:
            ...
            if response.data:
                ...
                logger.warning(
                    f"No data returned for innovation update {innovation_id}"
                )
    ...
    def estimate_manual_cost_savings(
        self, backfill_jobs: List, targeted_results: List
    ) -> float:
        ...
        successful_backfills = len(
            [job for job in backfill_jobs if job.status.value == "completed"]
        )
        successful_targeted = len(targeted_results)

        total_time_saved = (
            successful_backfills + successful_targeted
        ) * time_saved_per_result
        manual_cost_saved = total_time_saved * manual_research_cost_per_hour

        ai_cost = sum(job.total_cost for job in backfill_jobs) + sum(
            result.search_cost for result in targeted_results
        )
    ...
    def get_backfill_status(self) -> Dict[str, Any]:
        ...
        return {
            "last_run": self.last_backfill_run.isoformat()
            if self.last_backfill_run
            else None,
            "next_run_due": (
                self.last_backfill_run + self.backfill_interval
            ).isoformat()
            if self.last_backfill_run
            else "now",
...
async def integrate_with_etl_pipeline():
    ...
    if backfill_result["status"] == "completed":
        logger.info(
            f"AI backfill completed: {backfill_result['innovations_processed']} innovations processed"
        )
        logger.info(
            f"Cost: ${backfill_result['total_cost']:.2f}, Savings: ${backfill_result['cost_savings_estimate']:.2f}"
        )
