...
class CitationsAnalysisService:
    ...
    def __init__(self):
        ...
        # African institution patterns for knowledge flow - now actively used
        self.african_institution_patterns = [
            r"University of (?:Cape Town|Witwatersrand|Stellenbosch|KwaZulu-Natal|Lagos|Nairobi|Ghana|Cairo|Tunis)",
            r"(?:Makerere|Nigerian|Kenyan|South African|Egyptian|Moroccan|Rwandan|Ugandan|Tanzanian) (?:University|Institute)",
            r"African (?:Institute|University|Centre|Academy) (?:of|for) (?:\w+)",
            r"(?:AIMS|African Institute for Mathematical Sciences)",
            r"(?:AMMI|African Masters of Machine Intelligence)",
            r"Council for Scientific and Industrial Research",
            r"(?:Ashesi|Strathmore|Covenant|Landmark) University",
            r"(?:iHub|Co-creation Hub|Growth Hub|TechHub)",
        ]
    ...
    # CITATION NETWORK ANALYSIS
    async def extract_citations_from_publications(
    ...
    async def _extract_citations_from_text(
        self, paper_id: str, text: str, title: str
    ) -> List[CitationRelation]:
    ...
    async def _resolve_citation_reference(
        self, citing_id: str, ref_text: str, context: str, full_text: str
                ...
                for paper in similar_papers:
                    if paper.score > 0.85:  # High similarity threshold
                        return CitationRelation(
                            ...
                            * 0.7,  # Reduce confidence for fuzzy match
                            extracted_at=datetime.now(),
                        )
        ...
        try:
            ...
            if citation_data:
                ...
                if hasattr(response, "data") and response.data:
                    ...
                    logger.warning(
                        f"Failed to store {len(citation_data)} citation relationships"
                    )
    ...
    async def calculate_publication_impact_scores(self) -> Dict[str, ImpactMetrics]:
        ...
        try:
            ...
            citations_response = (
                self.supabase.table("citation_relationships").select("*").execute()
            )
    ...
    async def _calculate_individual_impact(
        self, pub_id: str, pub: Dict, citation_graph: Dict, citation_counts: Dict
    ) -> ImpactMetrics:
        ...
        try:
            ...
            industry_flow = await self._calculate_industry_flow_score(
                pub_id, citing_papers
            )
    ...
    async def _count_downstream_innovations(self, pub_id: str) -> int:
        ...
        try:
            ...
            response = (
                self.supabase.table("innovations").select("id, publications").execute()
            )
    ...
    async def _calculate_industry_flow_score(
        self, pub_id: str, citing_papers: List[str]
    ) -> float:
        ...
        try:
            ...
            for citing_id in citing_papers:
                ...
                response = (
                    self.supabase.table("publications")
                    .select("authors, abstract, content, affiliations")
                    .eq("id", citing_id)
                    .execute()
                )
                ...
                if response.data:
                    ...
                    # Check for African institution connections
                    african_connection = self._extract_african_institution_connections(
                        text
                    )
            ...
            african_boost = (
                african_citations / len(citing_papers)
            ) * 0.1  # 10% boost for African connections
    ...
    def _calculate_influence_score(
        ...
        normalized_scores = {
            ...
            "downstream": min(
                downstream / 10.0, 1.0
            ),  # Normalize to max 10 innovations
    ...
    async def _store_impact_metrics(self, impact_metrics: Dict[str, ImpactMetrics]):
        ...
        try:
            ...
            if metrics_data:
                ...
                logger.info(
                    f"Stored impact metrics for {len(metrics_data)} publications"
                )
    ...
    async def map_research_to_innovation_flows(self) -> List[KnowledgeFlowPath]:
        ...
        try:
            ...
            pubs_response = (
                self.supabase.table("publications")
                .select("id, title, abstract, publication_date, keywords")
                .execute()
            )

            innovations_response = (
                self.supabase.table("innovations")
                .select("id, title, description, creation_date, publications, tags")
                .execute()
            )
    ...
    async def _identify_knowledge_sources(
        self, innovation: Dict, publications: List[Dict]
    ) -> List[KnowledgeFlowPath]:
        """Identify research sources that contributed to an innovation"""
        flows = []

        try:
            innovation_text = (
                innovation.get("title", "") + " " + innovation.get("description", "")
            )
    ...
    def _classify_transformation_type(
        self, similarity: float, time_to_market: int
    ) -> str:
    ...
    async def _store_knowledge_flows(self, flows: List[KnowledgeFlowPath]):
        ...
        try:
            ...
            if flow_data:
                response = (
                    self.supabase.table("knowledge_flows").insert(flow_data).execute()
                )
                # Check if the insert was successful
                if hasattr(response, "data") and response.data:
                    ...
                    logger.warning(
                        f"Failed to store {len(flow_data)} knowledge flow paths"
                    )
    ...
    async def generate_citation_network_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pubs_response = (
                self.supabase.table("publications")
                .select("id, title, impact_metrics, publication_date, authors")
                .execute()
            )
            ...
            return {
                "network_overview": {
                    ...
                    / max(total_publications, 1),
                },
                "influential_research": [
                    {
                        "id": paper["id"],
                        "title": paper["title"],
                        "influence_score": paper["impact_metrics"].get(
                            "influence_score", 0
                        ),
                        "citation_count": paper["impact_metrics"].get(
                            "citation_count", 0
                        ),
                    }
    ...
    def _analyze_citation_timeline(
        self, citations: List[Dict], publications: List[Dict]
    ) -> List[Dict]:
        """Analyze citation patterns over time"""
        try:
            pub_dates = {
                pub["id"]: pub.get("publication_date")
    ...
    def _calculate_network_statistics(
        ...
        try:
            ...
            connected_papers = len(
                [p for p in pub_ids if p in citation_graph or p in reverse_graph]
            )
            isolation_rate = (len(pub_ids) - connected_papers) / max(len(pub_ids), 1)

            # Average degree (citations per paper)
            avg_out_degree = sum(len(refs) for refs in citation_graph.values()) / max(
                len(pub_ids), 1
            )
            avg_in_degree = sum(len(cites) for cites in reverse_graph.values()) / max(
                len(pub_ids), 1
            )
    ...
    def _analyze_knowledge_flows(self, flows: List[Dict]) -> Dict:
        ...
        try:
            ...
            time_to_market = [
                f["time_to_market_days"] for f in flows if f.get("time_to_market_days")
            ]
            avg_time_to_market = (
                sum(time_to_market) / len(time_to_market) if time_to_market else 0
            )
            ...
            flow_strengths = [
                f["flow_strength"] for f in flows if f.get("flow_strength")
            ]
            avg_flow_strength = (
                sum(flow_strengths) / len(flow_strengths) if flow_strengths else 0
            )
            ...
            return {
                ...
                "research_to_innovation_rate": len(
                    flows
                ),  # Could be normalized by total publications
            }
    ...
    def _extract_african_institution_connections(self, text: str) -> List[str]:
        ...
        # Also check for general African geographic indicators
        african_countries = [
            "nigeria",
            "kenya",
            "south africa",
        ...
        return list(set(african_institutions))  # Remove duplicates
    ...
    async def analyze_african_research_networks(self) -> Dict[str, Any]:
        ...
        try:
            # Get publications with author affiliations
            pubs_response = (
                self.supabase.table("publications")
                .select("id, title, authors, affiliations, abstract, content")
                .execute()
            )
            ...
            for pub in publications:
                ...
                full_text = (
                    ...
                    + pub.get("content", "")
                    + " "
                    + " ".join(pub.get("affiliations", []))
                )

                african_connections = self._extract_african_institution_connections(
                    full_text
                )

                if african_connections:
                    african_network["total_african_publications"] += 1

                    # Count institutions
                    for institution in african_connections:
                        african_network["institutions"][institution] += 1

                    # Extract research areas from keywords/abstract
                    research_keywords = self._extract_research_keywords(
                        pub.get("abstract", "")
                    )
                    for keyword in research_keywords:
                        african_network["research_areas"][keyword] += 1

            # Convert defaultdicts to regular dicts for JSON serialization
            return {
                "total_african_publications": african_network[
                    "total_african_publications"
                ],
                "top_institutions": dict(
                    sorted(
                        african_network["institutions"].items(),
                        key=lambda x: x[1],
                        reverse=True,
                    )[:20]
                ),
                "top_research_areas": dict(
                    sorted(
                        african_network["research_areas"].items(),
                        key=lambda x: x[1],
                        reverse=True,
                    )[:15]
                ),
                "network_coverage": {
                    "unique_institutions": len(african_network["institutions"]),
                    "research_diversity": len(african_network["research_areas"]),
                },
                "analyzed_at": datetime.now().isoformat(),
            }

        except Exception as e:
            logger.error(f"Error analyzing African research networks: {e}")
            return {}
    ...
    def _extract_research_keywords(self, text: str) -> List[str]:
        ...
        # Common research keywords in AI and technology
        keyword_patterns = [
        ...
        for pattern in keyword_patterns:
            if re.search(pattern, text_lower):
                ...
                keyword = (
                    pattern.replace(r"\b", "").replace(r"s?", "s").replace(r"\?", "")
                )
