...
try:
    ...
    logger.warning(
        "fuzzywuzzy not installed. Install with: pip install fuzzywuzzy python-levenshtein"
    )
...
class ContentHasher:
    ...
    def create_content_hash(
        self, title: str, description: str, organization: str = ""
    ) -> str:
...
class DeduplicationService:
    ...
    async def check_publication_duplicates(
        ...
        if publication_data.get("title"):
            title_match = await self._check_title_similarity(
                publication_data["title"], "publications", threshold=0.85
            )
    ...
    async def check_innovation_duplicates(
        ...
        if innovation_data.get("source_url"):
            url_match = await self._check_url_duplicate(
                innovation_data["source_url"], "innovations"
            )
        ...
        if innovation_data.get("title"):
            title_match = await self._check_title_similarity(
                innovation_data["title"], "innovations", threshold=0.80
            )
            if title_match.is_duplicate:
                duplicates.append(title_match)
    ...
    async def check_innovation_duplicate(
        self, innovation_data: Dict[str, Any]
    ) -> DuplicateMatch:
    ...
    async def check_organization_duplicate(
        ...
        if org_data.get("website"):
            url_match = await self._check_url_duplicate(
                org_data["website"], "organizations"
            )
            if url_match.is_duplicate:
                return url_match
    ...
    async def _check_arxiv_id_duplicate(self, arxiv_id: str) -> DuplicateMatch:
        ...
        try:
            publications = await self.db_service.get_publications(
    ...
    async def _check_title_similarity(
        self, title: str, table_name: str, threshold: float = 0.85
    ) -> DuplicateMatch:
    ...
    async def _check_content_similarity(
        self, title: str, content: str, table_name: str, threshold: float = 0.80
    ) -> DuplicateMatch:
        ...
        try:
            ...
            for record in records:
                ...
                if record_title or record_content:
                    record_hash = self.content_hasher.create_content_hash(
    ...
    async def _check_organization_name(self, name: str) -> DuplicateMatch:
        ...
        try:
            ...
            organizations = (
                await self.db_service.client.table("organizations")
                .select("*")
                .ilike("name", f"%{name}%")
                .execute()
            )
