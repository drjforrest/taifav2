...
class PineconeAssistantService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Pinecone Assistant '{self.assistant_name}' initialized successfully"
            )
            return True
    ...
    async def upload_file(
        ...
        try:
            ...
            response = self.assistant.upload_file(file_path=file_path, timeout=timeout)
    ...
    async def upload_publication_content(
    ...
    async def upload_innovation_content(
    ...
    def _format_publication_content(self, publication: Dict[str, Any]) -> str:
        ...
        if publication.get("development_stage"):
            content_parts.append(
                f"DEVELOPMENT_STAGE: {publication['development_stage']}"
            )
    ...
    def _format_innovation_content(self, innovation: Dict[str, Any]) -> str:
        ...
        if innovation.get("solution_approach"):
            content_parts.append(
                f"SOLUTION_APPROACH: {innovation['solution_approach']}"
            )
        ...
        if innovation.get("organizations"):
            ...
            if isinstance(orgs, list):
                org_names = [
                    org.get("name", "") for org in orgs if isinstance(org, dict)
                ]
                if org_names:
        ...
        if innovation.get("individuals"):
            ...
            if isinstance(individuals, list):
                names = [
                    ind.get("name", "") for ind in individuals if isinstance(ind, dict)
                ]
                if names:
        ...
        if innovation.get("verification_status"):
            content_parts.append(
                f"VERIFICATION_STATUS: {innovation['verification_status']}"
            )

        return "\n\n".join(content_parts)

    async def enhance_message_with_innovation_context(
        self, message: str, innovation: Dict[str, Any]
    ) -> str:
...
"""
            ...
            if innovation.get("related_organizations"):
                org_names = [
                    org.get("name", "") for org in innovation["related_organizations"]
                ]
            ...
            if innovation.get("related_publications"):
                pub_titles = [
                    pub.get("title", "")[:100]
                    for pub in innovation["related_publications"]
                ]
