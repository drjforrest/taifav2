...
class Crawl4AIService:
    ...
    async def crawl_url(
        self, url: str, extract_innovations: bool = True
    ) -> CrawlResult:
        ...
        try:
                        ...
                        for innovation_data in extracted_data["innovations"]:
                            try:
                                innovation = CrawledInnovation(
                                    **innovation_data, source_url=url
                                )
            ...
            if hasattr(result, "links"):
                links = [
                    urljoin(url, link) for link in result.links.get("internal", [])
                ]
    ...
    def filter_african_innovations(
        self, crawl_results: List[CrawlResult]
    ) -> List[CrawledInnovation]:
...
if __name__ == "__main__":
    ...
    async def test_crawler():
        ...
        for innovation in innovations[:3]:
            ...
            print(
                f"African Relevance: {innovation.metadata.get('african_relevance_score', 0):.2f}"
            )
