...
class SystematicReviewProcessor:
    ...
    def process_study_row(self, row: pd.Series) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            ai_techniques = str(
                row.get("AI/Machine Learning Techniques Used", "")
            ).strip()
            ...
            african_entities = self.extract_african_entities(
                geographic_scope, venue, authors
            )

            # Calculate relevance scores
            african_score = self.calculate_african_relevance(
                title, geographic_scope, venue, authors
            )
    ...
    def extract_african_entities(
        self, geographic_scope: str, venue: str, authors: List[str]
    ) -> List[str]:
    ...
    def calculate_african_relevance(
        self, title: str, geographic_scope: str, venue: str, authors: List[str]
    ) -> float:
    ...
    def calculate_ai_relevance(
        self, title: str, project_domain: str, ai_techniques: str
    ) -> float:
    ...
    def get_summary_statistics(self, studies: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Average scores
        avg_african_score = (
            sum(s["african_relevance_score"] for s in studies) / total_studies
        )
        ...
        return {
            ...
            "top_domains": dict(
                list(sorted(domains.items(), key=lambda x: x[1], reverse=True))[:10]
            ),
            "top_african_entities": dict(
                list(sorted(entity_count.items(), key=lambda x: x[1], reverse=True))[
                    :10
                ]
            ),
            "top_keywords": dict(
                list(sorted(keyword_count.items(), key=lambda x: x[1], reverse=True))[
                    :10
                ]
            ),
...
async def main():
    ...
    if studies:
        ...
        try:
            ...
            for study in studies:
                try:
                    ...
                    publication_data = {
                        ...
                        "african_relevance_score": study.get(
                    ...
                    if success and stored_record:
                        ...
                        logger.info(
                            f"✅ Stored systematic review study ({action}): {study.get('title', 'Unknown')[:50]}..."
                        )
                    elif not success:
                        logger.info(
                            f"ℹ️ Systematic review study handling ({action}): {study.get('title', 'Unknown')[:50]}..."
                        )
            ...
            logger.info(
                f"✅ Stored {len(stored_publications)}/{len(studies)} studies in database"
            )
        ...
        for i, study in enumerate(studies[:5], 1):
            logger.info(f"\n{i}. {study['title']}")
            logger.info(
                f"   Authors: {', '.join(study['authors'][:3])}{'...' if len(study['authors']) > 3 else ''}"
            )
